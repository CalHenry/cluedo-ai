...
from game_engine import CluedoGameEngine, GameScenario


# MCP Tools Implementation
def create_game_tools(game_engine: CluedoGameEngine):
    """Creates the MCP tool functions with access to the game scenario"""
...
"""
    ...
    def get_witness_statement(witness_name: str) -> str:
        ...
        if witness_name not in game_engine.SUSPECTS:
            ...
            return f"ERROR: Unknown person '{witness_name}'. Available witnesses: {available}"
...
"""
    ...
    def get_timeline_entry(time_slot: str) -> str:
        ...
        if time_slot not in timeline:
            ...
            return f"ERROR: Invalid time slot '{time_slot}'. Available time slots: {available}"
    ...
    def validate_solution(suspect: str, weapon: str, location: str) -> dict:
        ...
        Args:
            suspect: Name of the suspected murderer
            weapon: Murder weapon used
            location: Room where the murder took place

        Returns:
            Validation results indicating if the solution is correct
        """
        if not game_engine.scenario:
            return {"error": "No active investigation. Game scenario not initialized."}

        return game_engine.scenario.validate_solution(suspect, weapon, location)

    # tools as a dict
    return {
        "get_crime_scene_details": get_crime_scene_details,
        "get_witness_statement": get_witness_statement,
        "get_forensic_evidence": get_forensic_evidence,
        "get_suspect_background": get_suspect_background,
        "get_timeline_entry": get_timeline_entry,
        "check_fingerprints": check_fingerprints,
        "verify_alibi": verify_alibi,
        "validate_solution": validate_solution,
    }


# Test the tools by running the file
if __name__ == "__main__":
    # Initialize game engine
    engine = CluedoGameEngine(seed=42)  # Use seed for reproducible testing
    scenario = engine.generate_scenario()

    print("Murder Mystery Generated!")
    print(
        f"Solution: {scenario.murderer} with the {scenario.murder_weapon} in the {scenario.murder_location}"
    )
    print(f"\nAvailable Evidence IDs: {list(scenario.forensic_evidence.keys())}")
    print(f"\nWitnesses: {list(scenario.witness_statements.keys())}")
    print(f"\nCrime Scenes: {list(scenario.crime_scene_evidence.keys())}")

    # Create tools
    tools = create_game_tools(engine)
    ...
    print(tools["get_crime_scene_details"](scenario.murder_location))

    print("\n2. Witness Interview:")
    print(tools["get_witness_statement"](scenario.murderer))

    print("\n3. Forensic Analysis:")
    print(tools["get_forensic_evidence"]("FOR_WEAPON_001"))

    print("\n4. Suspectbackground:")
    print(tools["get_suspect_background"](scenario.murderer))

    print("\n5. Timeline entry:")
    print(tools["get_timeline_entry"]("21:30"))

    print("\n6. Check fingerprints:")
    print(tools["check_fingerprints"](scenario.murder_weapon))

    print("\n7. Verify aliby:")
    print(tools["verify_alibi"](scenario.murderer, "21:30"))

    print("\nLast. Solution Validation (correct):")
    print(
        tools["validate_solution"](
            scenario.murderer, scenario.murder_weapon, scenario.murder_location
        )
    )

    print("\n5. Solution Validation (incorrect):")
    print(tools["validate_solution"]("Miss Scarlet", "Knife", "Study"))
